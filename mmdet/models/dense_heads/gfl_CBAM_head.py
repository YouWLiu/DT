...
class CBAMBlock(nn.Module):
    ...
    def __init__(self, channel=512, reduction=16):
        super().__init__()
        self.ca = ChannelAttention(channel=channel, reduction=reduction).to("cuda:0")
        self.sa = SpatialAttention().to("cuda:0")
...
class GFLCBAMHead(AnchorHead):
    ...
    def forward_single(self, x: Tensor, scale: Scale) -> Sequence[Tensor]:
        ...
        cls_feat = x
        reg_feat = x
        cbam = CBAMBlock(channel=256, reduction=8)
        for cls_conv in self.cls_convs:
            cls_feat = cbam(cls_feat)
            cls_feat = cls_conv(cls_feat)
        for reg_conv in self.reg_convs:
            reg_feat = cbam(reg_feat)
