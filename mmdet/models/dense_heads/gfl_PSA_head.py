...
class PSA(nn.Module):
    ...
    def __init__(self, channel=512, reduction=4, S=4):
        ...
        self.convs = []
        for i in range(S):
            self.convs.append(nn.Conv2d(channel // S, channel // S, kernel_size=2 * (i + 1) + 1, padding=i + 1).to("cuda:0"))

        self.se_blocks = []
        for i in range(S):
            self.se_blocks.append(nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(channel // S, channel // (S * reduction), kernel_size=1, bias=False).to("cuda:0"),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel // (S * reduction), channel // S, kernel_size=1, bias=False).to("cuda:0"),
                nn.Sigmoid()
            ))
...
class GFLPSAHead(AnchorHead):
    ...
    def forward_single(self, x: Tensor, scale: Scale) -> Sequence[Tensor]:
        ...
        for cls_conv in self.cls_convs:
            cls_feat = psa(cls_feat)
            cls_feat = cls_conv(cls_feat)
        for reg_conv in self.reg_convs:
            reg_feat = psa(reg_feat)
